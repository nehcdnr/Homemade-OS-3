...
static void _unmapPage_(PageManager *p, MemoryBlockManager *physical, void *linearAddress, size_t size, int checkAutoRelease){
	size_t s = size;
	while(s != 0){
		s -= PAGE_SIZE;
	...
	while(s != 0){
		s -= PAGE_SIZE;
		releaseInvalidatedPage(p, physical, ((uintptr_t)linearAddress) + s, checkAutoRelease);
	}
}
